...
function FeaturedCard({ project }: { project: ProjectType }) {
  return (
    <motion.article
      variants={item}
      className="group relative col-span-1 md:col-span-2 row-span-2 rounded-3xl overflow-hidden flex flex-col"
    >
      {/* Gradient border — transitions from subtle to vivid on hover */}
      <div className="absolute inset-0 rounded-3xl transition-opacity duration-500 opacity-60 group-hover:opacity-100 pointer-events-none"
        style={{ background: "linear-gradient(135deg,rgba(0,212,255,0.45) 0%,transparent 45%,rgba(167,139,250,0.35) 100%)", padding: 1 }}
        aria-hidden
      />
      {/* Card body */}
      <div className="relative flex flex-col flex-1 m-px rounded-[calc(1.5rem-1px)] bg-surface/80 backdrop-blur-sm p-7 sm:p-9 overflow-hidden">
        {/* Inner glow */}
        <div className="absolute inset-0 -z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-700" aria-hidden>
        ...
        {/* Title */}
        <div className="mb-6">
          <h3 className="font-display font-bold text-4xl sm:text-5xl lg:text-[3.5rem] tracking-tighter text-foreground leading-[0.92] mb-3">
            {project.title}
          </h3>
          <p className="font-mono text-sm text-accent">{project.tagline}</p>
        </div>

        <p className="text-base sm:text-lg text-muted-strong leading-relaxed max-w-2xl mb-4 text-pretty">
          {project.description}
        </p>

        {project.role && (
          <p className="text-sm text-muted mb-6">
            <span className="text-muted/50">Role — </span>{project.role}
          </p>
        )}
          ...
          <ul className="grid sm:grid-cols-2 gap-3 mb-8">
            {project.highlights.map((h) => (
              <li key={h} className="flex gap-2.5 text-sm text-muted-strong">
                <span className="mt-1.5 shrink-0 h-1.5 w-1.5 rounded-full bg-accent" aria-hidden />
                <span>{h}</span>
              </li>
            ))}
          </ul>
        ...
        <div className="mt-auto">
          <div className="flex flex-wrap gap-2 mb-6">
            {project.tags.map((tag) => (
              <span key={tag} className="rounded-full border border-border bg-background/60 px-3 py-1 text-xs font-mono text-muted-strong">
                {tag}
              </span>
            ))}
          </div>
...
function ProjectCard({ project, num }: { project: ProjectType; num: number }) {
  return (
    <motion.article
      variants={item}
      className="group relative rounded-2xl border border-border bg-surface/60 p-6 flex flex-col overflow-hidden transition-all duration-300 hover:border-accent/35 hover:bg-surface hover:-translate-y-1 hover:shadow-card-hover"
    >
      {/* Hover glow */}
      <div className="absolute -top-20 -right-20 w-52 h-52 rounded-full bg-accent/[0.07] blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10" aria-hidden />

      {/* Badge row */}
      <div className="flex items-center justify-between mb-4">
        <CategoryBadge category={project.category} />
        <span className="font-mono text-[0.6rem] text-muted/40 tabular-nums">
      ...
      {/* Title */}
      <div className="mb-3">
        <h3 className="font-display font-bold text-xl sm:text-2xl text-foreground tracking-tight leading-tight">
          {project.title}
        </h3>
      ...
      <div className="flex flex-wrap gap-1.5 mb-5">
        {project.tags.slice(0, 5).map((tag) => (
          <span key={tag} className="rounded-full border border-border bg-background/40 px-2.5 py-0.5 text-[0.67rem] font-mono text-muted-strong">
            {tag}
          </span>
        ))}
        {project.tags.length > 5 && (
          <span className="rounded-full border border-border/60 bg-background/30 px-2.5 py-0.5 text-[0.67rem] font-mono text-muted">
            +{project.tags.length - 5}
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-4 pt-4 border-t border-border/60">
        ...
        {!project.liveUrl && !project.githubUrl && (
          <span className="text-xs font-mono text-muted/60 italic">Private</span>
        )}
      </div>
    </motion.article>
  );
      ...
      <div className="max-w-7xl mx-auto">
        <motion.div variants={container} initial="hidden" animate={isInView ? "show" : "hidden"}>
          ...
          <motion.div variants={item} className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-12">
            <h2
              id="projects-heading"
              className="font-display font-bold text-balance text-4xl sm:text-5xl md:text-6xl tracking-tighter text-foreground max-w-xl"
            >
