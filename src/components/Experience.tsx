...
export function Experience() {
  ...
  return (
    <section
      id="experience"
      ref={ref}
      className="relative py-24 sm:py-36 px-5 sm:px-10 lg:px-20 border-t border-border/60 bg-background-2/40"
    ...
    >
      <div className="max-w-5xl mx-auto">
        <motion.div variants={container} initial="hidden" animate={isInView ? "show" : "hidden"}>
          ...
          >
            A track record of{" "}
            <span className="text-gradient-accent">production work</span>.
          </motion.h2>

          <motion.div variants={container} className="mt-14 sm:mt-16 relative">
            {/* Desktop timeline line */}
            <div
              className="absolute left-3.5 top-3 bottom-3 w-px bg-gradient-to-b from-accent/50 via-border-strong/60 to-transparent hidden md:block"
              aria-hidden
            />
            ...
            <div className="space-y-8 sm:space-y-10">
              {experience.map((entry, idx) => (
                ...
                >
                  ...
                  {/* Card */}
                  <div className="group rounded-2xl border border-border bg-surface/60 p-6 sm:p-7 hover:border-accent/30 hover:-translate-y-0.5 hover:shadow-card transition-all duration-300 overflow-hidden relative">
                    {/* Subtle hover glow */}
                    <div
                      className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10 rounded-2xl"
                      style={{ background: "radial-gradient(ellipse 80% 60% at 50% 0%, rgba(0,212,255,0.06), transparent 70%)" }}
                      aria-hidden
                    />
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3 mb-5">
                      <div>
                        ...
                        <h3 className="font-display font-bold text-2xl sm:text-3xl tracking-tight text-foreground">
                          {entry.role}
                        </h3>
                      </div>

                      {/* Desktop duration & location */}
